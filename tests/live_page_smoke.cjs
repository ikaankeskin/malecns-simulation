...
setImmediate(()=>{
  ...
  elements.get('start-productivity').onclick();
  assert.equal(vm.runInContext('world.tick',sandbox),800);
  assert.equal(vm.runInContext('playing',sandbox),false);
  assert.match(elements.get('mission-status').textContent,/0\/2 distinct gardens matured/);
  assert.equal(elements.get('playback-speed').value,'0.25');
  vm.runInContext("world.patches[0].planter=0;world.patches[0].stage='growing';world.patches[0].timer=40;world.soil.water.moisture[world.patches[0].soil_cell]=.1;MaleCNSSoil.observe(world.soil,world.patches,world.tick);draw()",sandbox);
  assert.equal(elements.get('find-thirsty').disabled,false);
  const savedReserve=vm.runInContext('world.soil.water.reserve',sandbox);
  elements.get('find-thirsty').onclick();
  assert.equal(vm.runInContext('world.soil.water.reserve',sandbox),savedReserve);
  assert.equal(elements.get('mission-water').disabled,false);
  elements.get('mission-water').onclick();
  assert.ok(vm.runInContext('world.soil.water.reserve',sandbox)<savedReserve);
  elements.get('retry-mission').onclick();
  assert.equal(vm.runInContext('world.mission.kind',sandbox),'productivity');
  assert.equal(vm.runInContext('world.tick',sandbox),800);
