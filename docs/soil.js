...
(function(root){
  ...
  function create(rules){
    ...
    return {n,half:rules.map_half,size:2*rules.map_half/n,cells:Array(n*n).fill(1),
  ...
  function growthBudget(soil,patches,growth,rules){
    ...
    demands.forEach((rows,i)=>{
      const requested=rows.reduce((s,r)=>s+r[1],0),fraction=requested?Math.min(1,soil.cells[i]/requested):1;
  ...
  function snapshot(soil,patches){
    ...
    return {...soil,cells:[...soil.cells],occupied_cells:occupied.length,
